...
class BasketVendorItemDetailScreen extends StatefulWidget {
  final BasketVendorItem basketVendorItem;

  const BasketVendorItemDetailScreen({
    super.key,
    required this.basketVendorItem,
  });

  @override
  State<BasketVendorItemDetailScreen> createState() =>
      _BasketVendorItemDetailScreenState();
}

class _BasketVendorItemDetailScreenState
    extends State<BasketVendorItemDetailScreen> {
  ...
  BasketVendor? _basketVendor;
  BasketHeader? _basketHeader;
  BasketVendorItem? _currentItem;
  ...
  void initState() {
    super.initState();
    _itemAvailable = widget.basketVendorItem.itemAvailableWithVendor;
    _rateController.text = widget.basketVendorItem.rate.toString();
    _rateBeforeTaxController.text =
        widget.basketVendorItem.rateBeforeTax.toString();
    _taxPercentController.text = widget.basketVendorItem.taxPercent.toString();
    _calculatePrices();
    ...
    _currentItem = widget.basketVendorItem;
    _loadData();
  }
  ...
  Future<void> _loadData() async {
    ...
    final basketVendor = await _dbHelper.getBasketVendor(widget.basketVendorItem.basketVendorUuid);
    final basketHeader = await _dbHelper.getBasketHeader(widget.basketVendorItem.basketUuid);

    setState(() {
      _basketVendor = basketVendor;
  ...
  void _calculatePrices() {
    final rateBeforeTax = double.tryParse(_rateBeforeTaxController.text) ?? 0.0;
    final taxPercent = double.tryParse(_taxPercentController.text) ?? 0.0;
    final quantity = widget.basketVendorItem.quantity;
  ...
  Future<void> _autoSave() async {
    ...
    await _dbHelper.updateBasketVendorItem(item);
    _currentItem = item;
  }
          ...
          title: _isLoading
              ...
              : Row(
                  ...
                  children: [
                    ...
                    Text(
                      _basketVendor?.id != null ? '#${_basketVendor!.id}' : '...',
                      style: const TextStyle(fontSize: 18),
                    ),
                    const SizedBox(width: 8),
                    const Text('|', style: TextStyle(fontSize: 18)),
                    ...
                    const SizedBox(width: 4),
                    Text(
                      widget.basketVendorItem.id != null ? '#${widget.basketVendorItem.id}' : '...',
                      style: const TextStyle(fontSize: 18),
                    ),
                  ],
                ),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow(
                          'Model:', widget.basketVendorItem.model ?? ''),
                      _buildInfoRow(
                        'Quantity:',
                        widget.basketVendorItem.quantity.toStringAsFixed(2),
                      ),
                      if (widget.basketVendorItem.maxRetailPrice != null)
                        _buildInfoRow(
                          'MRP per unit:',
                          '${widget.basketVendorItem.currency} ${widget.basketVendorItem.maxRetailPrice!.toStringAsFixed(2)}',
                        ),
                    ],
                  ),
                ),
              ...
              Card(
                color: _itemAvailable ? Colors.green.shade50 : Colors.red.shade50,
                child: ListTile(
                  leading: Icon(
                    _itemAvailable ? Icons.check_circle : Icons.cancel,
                    color: _itemAvailable ? Colors.green.shade700 : Colors.red,
                  ),
                  title: const Text(
                    ...
                    Expanded(
                      ...
                      child: TextFormField(
                        ...
                        controller: TextEditingController(
                          text: widget.basketVendorItem.currency,
                        ),
                      ),
                    ),
                  ],
                  ...
                  children: [
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        controller: TextEditingController(
                          text: _calculatedTaxAmount.toStringAsFixed(2),
                        ),
                      ),
                    ),
                  ],
                  ...
                  children: [
                    Expanded(
                      child: TextFormField(
                        ...
                        controller: TextEditingController(
                          text: _calculatedBasePrice.toStringAsFixed(2),
                        ),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: TextFormField(
                        ...
                        controller: TextEditingController(
                          text: _calculatedTotalAmount.toStringAsFixed(2),
                        ),
                      ),
                    ),
                  ],
