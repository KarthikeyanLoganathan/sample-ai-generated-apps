const changeLog = {
    ...
    initializeChangeLogFromDataSheets() {
        ...
        const changeLogSheet = ss.getSheetByName(TABLE_DEFINITIONS.change_log.NAME);
        const now = new Date();

        if (!changeLogSheet) {
            throw new Error(`${TABLE_DEFINITIONS.change_log.NAME} sheet not found. Run setup() first.`);
        }

        // Clear all data except header row
        cleanup.cleanupSheet(TABLE_DEFINITIONS.change_log.NAME);
        ...
        for (const sheetName in TABLE_NAMES_TO_INDICES) {
            ...
            const colIdx = TABLE_META_INFO[sheetName].COLUMN_INDICES;
            ...
            const tableKeyUuidValues = sheet.getRange(2, colIdx.uuid + 1, sheetLastRow - 1, 1).getValues();
            // Read updated_at column values (skip header row)
            const updatedAtValues = sheet.getRange(2, colIdx.updated_at + 1, sheetLastRow - 1, 1).getValues();

            // Prepare data for change_log
            for (let i = 0; i < tableKeyUuidValues.length; i++) {
                const table_key_uuid = tableKeyUuidValues[i][0];
                const updatedAt = updatedAtValues[i][0];
                if (table_key_uuid && typeof table_key_uuid === "string" && table_key_uuid.trim() !== "") {
                    dataTable.push([utils.UUID(), tableIndex, table_key_uuid, CHANGE_MODE_INSERT, updatedAt || now]);
                }
            }

            Logger.log(`Processed ${tableKeyUuidValues.length} records from "${sheetName}"`);
        }

        // Insert all records into change_log
        if (dataTable.length > 0) {
            changeLogSheet.getRange(2, 1, dataTable.length,
                TABLE_META_INFO[TABLE_DEFINITIONS.change_log.NAME].COLUMN_COUNT
            ).setValues(dataTable);
        }

        setup.setupDataTableSheet(TABLE_DEFINITIONS.change_log.NAME, false);
     ...
     * Keeps only the first INSERT/UPDATE for each table_key_uuid and removes entries that were later deleted
    ...
    * @returns {array} Array of condensed change records
    ...
    prepareCondensedChangeLogFromChangeLog(sinceEpochTimeMilliseconds) {
        const ss = SpreadsheetApp.getActiveSpreadsheet();
        const sheet = ss.getSheetByName(TABLE_DEFINITIONS.change_log.NAME);
        const colIdx = TABLE_META_INFO?.[TABLE_DEFINITIONS.change_log.NAME].COLUMN_INDICES;
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            const rec = {
                ...
                table_key_uuid: row[colIdx.table_key_uuid],
            ...
            const table_key_uuid = rec.table_key_uuid;

            if (rec.change_mode === CHANGE_MODE_INSERT || rec.change_mode === CHANGE_MODE_UPDATE) {
                const oldRec = tableChangeHistory[changeHistoryTableMemberName][table_key_uuid]
                if (!oldRec) {
                    tableChangeHistory[changeHistoryTableMemberName][table_key_uuid] = rec;
                }
                ...
                if (oldRec && (oldRec.change_mode === CHANGE_MODE_INSERT || oldRec.change_mode === CHANGE_MODE_UPDATE)) {
                    delete tableChangeHistory[changeHistoryTableMemberName][table_key_uuid];
        ...
        for (const changeHistoryTableMemberName in tableChangeHistory) {
            ...
            for (const table_key_uuid in tableHistory) {
                result.push(tableHistory[table_key_uuid]);
            }
        }
    ...
    * @returns {string} Status message with count of records written
    ...
    writeCondensedChangeLog(sinceEpochTimeMilliseconds) {
        ...
        cleanup.cleanupSheet(TABLE_DEFINITIONS.condensed_change_log.NAME);
        const ss = SpreadsheetApp.getActiveSpreadsheet();
        ...
        let condensedSheet = ss.getSheetByName(TABLE_DEFINITIONS.condensed_change_log.NAME);
        if (!condensedSheet) {
            Logger.log(`${TABLE_DEFINITIONS.condensed_change_log.NAME} sheet does not exist`);
        }
        ...
        if (condensedData.length > 0) {
            ...
            condensedSheet.getRange(2, 1, dataTable.length,
                TABLE_META_INFO[TABLE_DEFINITIONS.condensed_change_log.NAME].COLUMN_COUNT
            ).setValues(dataTable);
        }
        //setup.setupDataTableSheet(TABLE_DEFINITIONS.condensed_change_log.NAME, false);
        Logger.log(`Wrote ${condensedData.length} records to ${TABLE_DEFINITIONS.condensed_change_log.NAME}`);
        return `Wrote ${condensedData.length} records to ${TABLE_DEFINITIONS.condensed_change_log.NAME}`;
    },
    ...
    readCondensedChangeLog(offset, limit) {
        const ss = SpreadsheetApp.getActiveSpreadsheet();
        const sheet = ss.getSheetByName(TABLE_DEFINITIONS.condensed_change_log.NAME);
        const tableMetaInfo = TABLE_META_INFO?.[TABLE_DEFINITIONS.condensed_change_log.NAME];
        ...
        // Read the data range
        const data = sheet.getRange(startRow, 1, numRows, tableMetaInfo.COLUMN_COUNT).getValues();
     ...
     * @param {string} table_key_uuid - UUID of the record
     * @param {string} changeMode - 'I', 'U', or 'D'
     * @param {Date} updatedAt - Timestamp of the update
     */
    logChange(tableName, table_key_uuid, changeMode, updatedAt) {
        try {
            const ss = SpreadsheetApp.getActiveSpreadsheet();
            const changeLogSheet = ss.getSheetByName(TABLE_DEFINITIONS.change_log.NAME);
            ...
            const now = new Date();
            changeLogSheet.appendRow([utils.UUID(), tableIndex, table_key_uuid, changeMode, updatedAt || now]);
        } catch (error) {
     ...
     * @param {string} tableName - Name of the table
     * @param {string[]} table_key_uuids - Array of UUIDs of the records
     * @param {string} changeMode - 'I', 'U', or 'D'
     * @param {Date} updatedAt - Timestamp of the update
     */
    logChanges(tableName, table_key_uuids, changeMode, updatedAt) {
        try {
            if (!table_key_uuids || table_key_uuids.length === 0) {
                return;
            }
            ...
            const rows = table_key_uuids.map(table_key_uuid => [utils.UUID(), tableIndex, table_key_uuid, changeMode, now]);
    ...
    insertRecord(tableIndex, table_key_uuid, changeMode, updatedAt) {
        const ss = SpreadsheetApp.getActiveSpreadsheet();
        const changeLogSheet = ss.getSheetByName(TABLE_DEFINITIONS.change_log.NAME);
