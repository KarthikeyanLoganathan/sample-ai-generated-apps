const maintainManufacturerModelNames = {
    ...
    saveData() {
        ...
        const columnMap = utils.getSheetColumnMap(mmSheet, TABLE_DEFINITIONS.manufacturer_materials.NAME);
        const keyColumnIndex = columnMap[TABLE_DEFINITIONS.manufacturer_materials.KEY_COLUMN];
        ...
        if (newEntries.length > 0) {
            const insertData = newEntries.map(rec => {
                return utils.buildRowDataFromRecord(rec, tableMetaInfo.COLUMN_NAMES, columnMap, TABLE_DEFINITIONS.manufacturer_materials.NAME);
            });
