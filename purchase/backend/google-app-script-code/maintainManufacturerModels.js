const maintainManufacturerModelNames = {
    ...
    saveData() {
        ...
        const mmSheet = ss.getSheetByName(TABLE_DEFINITIONS.manufacturer_materials.NAME);
        ...
        const tableMetaInfo = TABLE_META_INFO?.[TABLE_DEFINITIONS.manufacturer_materials.NAME];
        ...
        if (deleteEntries.length > 0) {
            ...
            changeLog.logChanges(TABLE_DEFINITIONS.manufacturer_materials.NAME, 
        ...
        if (newEntries.length > 0) {
            const insertData = newEntries.map(rec => {
                const row = new Array(tableMetaInfo?.COLUMN_COUNT).fill("");
                row[colIdx.uuid] = rec.uuid;
                row[colIdx.manufacturer_uuid] = rec.manufacturer_uuid;
                row[colIdx.material_uuid] = rec.material_uuid;
                row[colIdx.model] = rec.model;
                row[colIdx.updated_at] = rec.updated_at;
                return row;
            });

            const startRow = mmSheet.getLastRow() + 1;
            mmSheet.getRange(startRow, 1, insertData.length, tableMetaInfo?.COLUMN_COUNT)
        ...
        setup.setupDataTableSheet(TABLE_DEFINITIONS.manufacturer_materials.NAME, false);
