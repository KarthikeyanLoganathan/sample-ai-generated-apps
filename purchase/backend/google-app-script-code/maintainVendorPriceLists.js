...
const maintainVendorPriceLists = {
    ...
    setupSheet() {
        ...
        utils.createDataValidationForGivenRange(
            ...
            TABLE_DEFINITIONS.vendors.NAME,
            TABLE_META_INFO?.[TABLE_DEFINITIONS.vendors.NAME]?.COLUMNS?.name,
            2);
        ...
        utils.createDataValidationForGivenRange(
            ...
            TABLE_DEFINITIONS.manufacturers.NAME,
            TABLE_META_INFO?.[TABLE_DEFINITIONS.manufacturers.NAME]?.COLUMNS?.name,
            2);
        ...
        utils.createDataValidationForGivenRange(
            ...
            TABLE_DEFINITIONS.materials.NAME,
            TABLE_META_INFO?.[TABLE_DEFINITIONS.materials.NAME]?.COLUMNS?.name,
            2);
    ...
    saveData() {
        ...
        if (updatedEntries.length > 0) {
            Logger.log(`Upserting ${updatedEntries.length} records to ${TABLE_DEFINITIONS.vendor_price_lists.NAME}`);
            const upserted = deltaSync.upsertRecords(TABLE_DEFINITIONS.vendor_price_lists.NAME, updatedEntries);
            // Log changes
            changeLog.logChanges(TABLE_DEFINITIONS.vendor_price_lists.NAME, updatedEntries.map(rec => rec.uuid), CHANGE_MODE_UPDATE, currentTime);
        }
        if (newEntries.length > 0) {
            Logger.log(`Upserting ${newEntries.length} records to ${TABLE_DEFINITIONS.vendor_price_lists.NAME}`);
            const upserted = deltaSync.upsertRecords(TABLE_DEFINITIONS.vendor_price_lists.NAME, newEntries);
            // Log changes
            changeLog.logChanges(TABLE_DEFINITIONS.vendor_price_lists.NAME, newEntries.map(rec => rec.uuid), CHANGE_MODE_INSERT, currentTime);
        }

        // Apply formatting and resize
        setup.setupDataTableSheet(TABLE_DEFINITIONS.vendor_price_lists.NAME, false);
