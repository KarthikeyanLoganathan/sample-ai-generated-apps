const maintainManufacturerModelData = {
    ...
    saveData() {
        ...
        if (entries.length > 0) {
            Logger.log(`Upserting ${entries.length} records to ${TABLE_DEFINITIONS.manufacturer_materials.NAME}`);
            const upserted = deltaSync.upsertRecords(TABLE_DEFINITIONS.manufacturer_materials.NAME, entries);
            // Log changes
            changeLog.logChanges(TABLE_DEFINITIONS.manufacturer_materials.NAME, entries.map(rec => rec.uuid), CHANGE_MODE_UPDATE, currentTime);
        }
